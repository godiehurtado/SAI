...
-- Description:	Normaliza y homologa la información de Recaudos entregada por SISE VIDA.
--				Se agrega la validación de homologación sobre BancoDetalle
-- =============================================
CREATE PROCEDURE [dbo].[SAI_Recaudostemp_SISEVIDA]
AS
BEGIN
	...
	-- Actualización con el id del Amparo relacionado el AmparoDetalle que se envia en el archivo de Recaudos de SISE VIDA
	UPDATE recaudostempVIDA
	SET amparo_id = a.id
	FROM RecaudostempVIDA AS rt
	...
	-- Actualización con el id de la Cobertura relacionada al codigoCore enviado en el archivo de Recaudos de SISE VIDA
	UPDATE recaudostempVIDA
	...
	-- Actualización con el id del Plan relacionado al codigoCore del plANDetalle enviado en el archivo de recaudos de SISE VIDA
	UPDATE recaudostempVIDA
	SET plan_id = pld.id
	FROM RecaudostempVIDA AS rt
	INNER JOIN RamoDetalle rd ON rd.id = CAST(rt.ramo_id AS INT)
	...
	-- Actualización con el id de la localidad SAI respecto al codigo SISE VIDA
	UPDATE recaudostempVIDA
	SET localidad_id = l.id
	FROM RecaudostempVIDA AS rt
	INNER JOIN Localidad AS l ON CAST(l.[codigo SISE] AS INT) = CAST(rt.localidad_id AS INT)

	-- Actualización con el id del BancoDetalle respecto al codigoCore de SISE VIDA
	UPDATE recaudostempVIDA
	...
	WHERE bd.compania_id = 2
		AND rt.codigoBanco <> ''

	-- Actualización con el id de la Red respecto al codigoCore de VIDA Y la Agrupacion de Redes x Compañía
	UPDATE recaudostempVIDA
	SET red_id = rd.id
	FROM RecaudostempVIDA AS rt
