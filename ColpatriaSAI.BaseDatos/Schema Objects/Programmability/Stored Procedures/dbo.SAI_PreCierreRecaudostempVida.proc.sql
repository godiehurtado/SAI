CREATE PROCEDURE [dbo].[SAI_PreCierreRecaudostempVida]
...
--  Valida si hay información en RecaudostempVida.
	IF EXISTS (SELECT * FROM RecaudostempVida WHERE compania_id = @compania_id AND sistema = @sistema)
	BEGIN
	...
	--  Actualiza los registros de la compania a estado abierto.
		...
		--  Si existen registros en RecaudostempVida éstos se actualizan a estado cerrado (2)
			UPDATE RecaudostempVida
			SET estadoCierre = 2
			, mesCierre = @mesCierre
