...
-- =============================================
CREATE PROCEDURE [dbo].[CalcularEjecucionPresupuestoExcepciones]
...
BEGIN
	...
	Create Index i6 ON #ConsolidadoMesTemp(participante_id)
	...
	INSERT INTO #ConsolidadoMesTemp (id, compania_id, ramo_id, producto_id, clave, participante_id, lineaNegocio_id, amparo_id, modalidadPago_id, segmento_id, tipoMedida_id, Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre, Diciembre, año)
	SELECT
		id,      
		compania_id,
		ramo_id,
	...
	FROM
		(
			SELECT sub1.jerarquiaDetalle_id, sub1.meta_id
			FROM
			(
				SELECT jerarquiaDetalle_id, meta_id 
				FROM detallepresupuesto 
				WHERE presupuesto_id = @idPresupuesto  
				GROUP BY jerarquiaDetalle_id, meta_id 
			) as sub1
			INNER JOIN
			(
			...
			) as sub2 ON sub1.jerarquiaDetalle_id = sub2.excepcionJerarquiaOrigen_id AND sub1.meta_id = sub2.meta_id
		)as dp
		INNER JOIN meta as m ON dp.meta_id = m.id
	...
	-- DROP TABLE #DetallePresupuestoExcepciones
	CREATE TABLE #DetallePresupuestoExcepciones(
		jerarquiaDetalle_id int NOT NULL,
		excepcionJerarquiaDestino_id int NULL	
	)
	
    Create Index i1 ON #DetallePresupuestoExcepciones(jerarquiaDetalle_id)
	Create Index i2 ON #DetallePresupuestoExcepciones(excepcionJerarquiaDestino_id)	
	
	INSERT INTO #DetallePresupuestoExcepciones(jerarquiaDetalle_id,excepcionJerarquiaDestino_id)
	...
	-- DROP TABLE #NodosExcepcionesHijos
	CREATE TABLE #NodosExcepcionesHijos(
		jerarquiaDetalle_id int NOT NULL,
		clave nvarchar(100) COLLATE Modern_Spanish_CI_AS NULL,
    ...
    Create Index i1 ON #NodosExcepcionesHijos(jerarquiaDetalle_id)
	Create Index i2 ON #NodosExcepcionesHijos(clave)	
	Create Index i3 ON #NodosExcepcionesHijos(participante_id)	

	INSERT INTO #NodosExcepcionesHijos(jerarquiaDetalle_id,clave,jerarquia_id,canal_id,participante_id)
	SELECT dp.jerarquiaDetalle_id,hijos.clave,hijos.jerarquia_id,hijos.canal_id,participante_id
	FROM
	#DetallePresupuestoExcepciones as dp
	CROSS APPLY ObtenerHijosNodo(dp.excepcionJerarquiaDestino_id) as hijos

	TRUNCATE TABLE ConsolidadoMesTemp
	...
	(
		...
		SELECT
			...
			c.Diciembre
		FROM 
			#NodosExcepcionesHijos as hijos
			INNER JOIN #ConsolidadoMesTemp AS c ON hijos.participante_id = c.participante_id
