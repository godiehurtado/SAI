...
-- =============================================
CREATE PROCEDURE [dbo].[RPT_TotalEjecutadoXNodo]
...
BEGIN
	...
	INSERT INTO #ConsolidadoMes (participante_id,compania_id, ramo_id, producto_id, lineaNegocio_id, amparo_id, modalidadPago_id, segmento_id, tipoMedida_id, Enero, Febrero, Marzo, Abril, Mayo, Junio, Julio, Agosto, Septiembre, Octubre, Noviembre, Diciembre, año)
	SELECT 
	...
	INNER JOIN ConsolidadoMes as cm ON sub1.participante_id = cm.participante_id	
	WHERE cm.año = @anio
	GROUP BY sub1.participante_id,
		cm.compania_id,
		cm.ramo_id,
		cm.producto_id,
		cm.lineanegocio_id,
	...
	WHERE tm.esMeta = 1		
	ORDER BY jd.codigoNivel,c.nombre,r.nombre,p.nombre,ln.nombre,tm.nombre		
