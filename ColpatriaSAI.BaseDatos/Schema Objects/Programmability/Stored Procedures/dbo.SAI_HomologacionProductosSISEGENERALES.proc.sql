...
-- Description:	Homologación del archivo de Productos de GENERALES. Se toma la información por columnas (concepto) y se
--			    inserta en la tabla respectiva.
-- =============================================
CREATE PROCEDURE [dbo].[SAI_HomologacionProductosSISEGENERALES]
AS
BEGIN

	UPDATE Productotemp
	SET Compania = 'SISE VIDA'
	WHERE Compania = 'SISE GENERALES'
		AND Cod_Compania = '2'
	...
	WHERE s.Cod_Ramo <> ''
		...
		AND s.Cod_Compania = '1'
		AND s.Compania = 'SISE GENERALES'
		AND NOT EXISTS (
			SELECT *
