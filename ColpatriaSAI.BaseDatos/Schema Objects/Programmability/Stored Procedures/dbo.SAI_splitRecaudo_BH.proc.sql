...
-- =============================================
CREATE PROCEDURE [dbo].[SAI_splitRecaudo_BH]
AS
BEGIN
	DECLARE @cantidadRegistros AS INT = (SELECT COUNT(*) FROM RecaudostempBH)
	...
	BEGIN
		...
		CREATE TABLE #RecaudostempsplitBH (
		...
		-- *********************************************************************************************************--
		INSERT INTO #RecaudostempsplitBH (
			clave
			...
			,rte.estadoCierre
			,rte.mesCierre
			,rte.anioCierre FROM RecaudostempErrorBH rte INNER JOIN Compania c ON c.id = rte.compania_id INNER JOIN RamoDetalle rd ON rd.id = rte.ramo_id INNER JOIN ProductoDetalle pd ON pd.id = rte.producto_id INNER JOIN Localidad l ON l.id = rte.localidad_id INNER JOIN LineaNegocio ln ON ln.id = rte.lineaNegocio_id WHERE c.id = rd.compania_id
			AND c.id = 5
			AND rte.clave <> ''
		...
		CREATE TABLE #Recaudostemp1splitBH (
			clave NVARCHAR(2048)
			,porcentajeParticipacion NVARCHAR(2048)
			,numeroNegocio NVARCHAR(2048)
			,compania NVARCHAR(2048)
		...
		-- *********************************************************************************************************--
		DECLARE @total AS INT = (SELECT MAX(nte.id) FROM #RecaudostempsplitBH nte)
		DECLARE @inicio AS INT = (SELECT MIN(nte.id) FROM #RecaudostempsplitBH nte)
		...
		BEGIN
			...
			BEGIN
				...
				-- *********************************************************************************************************--
				INSERT INTO #Recaudostemp1splitBH (
					clave
					,porcentajeParticipacion
					,numeroNegocio
					,compania
		...
		INSERT INTO Recaudo (
			...
			,estadoCierre
			,mesCierre
			,anioCierre FROM #Recaudostemp1splitBH
			)

		DROP TABLE #RecaudostempsplitBH

		DROP TABLE #Recaudostemp1splitBH
