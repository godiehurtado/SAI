/*CREATE PROCEDURE [dbo].[SAI_Negociostemp_SISEGENERALES_CONSNEG]
AS
BEGIN

DECLARE @añoVigente AS INT = (SELECT CAST(p.valor AS INT) FROM ParametrosApp p WHERE p.id = 3)

CREATE TABLE [dbo].[#NegocioGen](
...
From  Negocio
Where Negocio.compania_id = 1 AND Negocio.sistema = 'SISE GENERALES' AND YEAR(fechaExpedicion) = @añoVigente
...
delete from Negocio where compania_id = 1 AND sistema = 'SISE GENERALES' AND YEAR(fechaExpedicion) >= @añoVigente
...
drop table #NegocioGen

update Negocio set actividadEconomica_id = 0 where actividadEconomica_id is NULL and Negocio.compania_id = 1
