...
CREATE PROCEDURE [dbo].[RPT_PersistenciaCAPI_EjecutivoxPeriodo] @localidad_id AS INT = 0
	,@zona_id AS INT = 0
	,@canal_id INT = 0
	,@nivel_id INT = 0
...
BEGIN
	SELECT z.nombre AS zona
		...
		,pl.nombre AS plazo
		,pc.persistenciaPeriodo
		,pc.anioCierreNegocio
		,jd.codigoNivel AS codigoNivel
		,jd.nombre AS nombreejecutivo
		,c.nombre AS canal
		,n.nombre AS nivel
	FROM PersistenciadeCAPIPeriodo pc
	LEFT JOIN JerarquiaDetalle jd ON jd.id = pc.jerarquiaDetalle_id
	...
	ORDER BY pc.jerarquiaDetalle_id
		,pc.periodo
END
