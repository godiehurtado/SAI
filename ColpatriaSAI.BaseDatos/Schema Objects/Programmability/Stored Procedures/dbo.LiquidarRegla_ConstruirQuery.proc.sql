...
-- =============================================
CREATE PROCEDURE [dbo].[LiquidarRegla_ConstruirQuery]
	@subregla_id	int,
	@fechaInicio	datetime,
	@fechaFinal		datetime
AS
BEGIN
	--DECLARE @subregla_id int=1019, @fechaInicio datetime='2011-03-01', @fechaFinal datetime='2011-03-31'
	...
	DECLARE @tipoConcurso int = ( SELECT C.tipoConcurso_id FROM SubRegla SR JOIN Regla R ON R.id=SR.regla_id JOIN Concurso C ON C.id=R.concurso_id WHERE SR.id=@subregla_id )
	...
	WHERE sr.regla_id = (SELECT regla_id FROM SubRegla WHERE id=@idSubregla) AND v.esFiltro=0 GROUP BY v.id
	IF CHARINDEX(N'13,', @var)<>0  SET @filtrarAnioAnterior=1
	
	EXEC dbo.LiquidarRegla_GenerarTablasFiltradas @crearTipoTabla1, @crearTipoTabla2, @crearTipoTabla3, @crearTipoTabla4, @fecha_inicio, @fecha_final, @filtrarAnioAnterior
